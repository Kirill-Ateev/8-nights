...
class NftCollection {
    ...
    createChangeContentBody(params) {
        const body = (0, ton_core_1.beginCell)();
        body.storeUint(4, 32); // Operation code for changing content
        // Store new collection content
        const newCollectionContent = (0, utils_1.encodeOffChainContent)(params.collectionContentUrl);
        body.storeRef(newCollectionContent);
        // Store new common content
        const newCommonContent = (0, ton_core_1.beginCell)();
        newCommonContent.storeBuffer(Buffer.from(params.commonContentUrl));
        body.storeRef(newCommonContent.endCell());
        return body.endCell();
    }
    changeContent(wallet, params) {
        return __awaiter(this, void 0, void 0, function* () {
            const seqno = yield wallet.contract.getSeqno();
            const changeContentBody = this.createChangeContentBody(params);
            yield wallet.contract.sendTransfer({
                seqno,
                secretKey: wallet.keyPair.secretKey,
                messages: [
                    (0, ton_core_1.internal)({
                        value: '0.03', // Adjust the value as required by the contract for this operation
                        to: this.address,
