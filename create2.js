...
// Add a function to generate random words for speech
...
function generateSound(duration, intensity) {
  ...
  if (Math.random() < 0.1) {
    // 10% chance to add a voice
    const word = getRandomWord();
    say.export(word, null, 1.0, `voices/${word}.wav`, (err) => {
      if (err) {
        console.error(err);
      } else {
        console.log(`Generated voice for word: ${word}`);
      }
    });
  }
