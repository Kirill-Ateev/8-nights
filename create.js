...
async function generatePiece(numSounds, duration) {
  const output = [];
  ...
  for (let i = 1; i <= numSounds; i++) {
    ...
    await new Promise((resolve, reject) => {
      ffmpeg()
        ...
        .size('128x128') // Be careful with the resolution, as you may not always get a broken image
        .toFormat('mp4')
        .on('end', resolve)
        .on('error', reject)
    ...
    output.push({
      name: `Night №${i}`,
      image: i,
      description: `This is a generative multimedia collection of sounds from which the picture is generated and compressed. Everything together creates a unique atmosphere of the night experience. Night shards crumbs settle in the consciousness. The sheer magnitude of knowledge is staggering in the variability of being. Always and in everything. Observing, it is reflected in us, your favorite attention wrappers. Night №${i} by Kirill Ateev`,
      attributes: [
        {
